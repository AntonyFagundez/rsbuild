...
`preview` retorna los siguientes parámetros:

- `urls`: URLs para acceder al servidor.
- `port`: El puerto donde actualente escucha los cambios.
...
Cuando la opción `target` de `createRsbuild` contiene solo un valor, el valor de retorno es `Compiler`; cuando `target` contiene múltiples valores, el valor de retorno es `MultiCompiler`.
...
> En la mayoría de los escenarios, no necesitas usar esta API a menos que necesites personalizar el servidor de desarrollo u otros escenarios avanzados.

## rsbuild.addPlugins

Registra uno o más plugins de Rsbuild, que pueden ser llamados múltiples veces.

Este método debe ser llamado antes de compilar. Si se llama después de compilar, no afectará el resultado de la compilación.
...
Obtener todos los plugins de Rsbuild registrados en la instancia actual de Rsbuild.
...
Eliminar uno o más plugins de Rsbuild, que pueden ser llamados múltiples veces.
...
El método initConfigs se utiliza para inicializar las configuraciones internas de Rsbuild y devolver la configuración de Rspack generada internamente por Rsbuild.

Generalmente, no necesitas llamar al método initConfigs, porque se llamará automáticamente cuando se llamen métodos como [rsbuild.build](#rsbuildbuild), [rsbuild.startDevServer](#rsbuildstartdevserver).
...
El método inspectConfig se utiliza típicamente para depurar la configuración interna de Rsbuild. Devuelve la configuración de Rsbuild y la configuración de Rspack generadas internamente, las serializa en cadenas y admite escribirlas en el disco.

Si necesitas ver las configuraciones de Rsbuild y Rspack durante el proceso de compilación, puedes usar el [modo de depuración](/guide/debug/debug-mode), o obtenerlas a través de ganchos como [onBeforeBuild](#rsbuildonbeforebuild), [onBeforeCreateCompiler](#rsbuildonbeforecreatecompiler).
...
Obtener el contenido de las configuraciones en formato de string:
...
Escribir el contenido de la configuración en el disco:
...
Puede establecer la ruta de salida usando `outputPath`. El valor predeterminado es [output.distPath.root](/config/output/dist-path).

Si `outputPath` es una ruta relativa, se concatenará en relación con el valor de `output.distPath.root`. También puede establecer `outputPath` en una ruta absoluta, en cuyo caso los archivos se escribirán directamente en esa ruta. Por ejemplo
